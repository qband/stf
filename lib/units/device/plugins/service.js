...
module.exports = syrup.serial()
  ...
  .define(function(options, adb, router, push, apk) {
    ...
    plugin.setWhiteList = function(text) {
      return runServiceCommand(
          apk.wire.MessageType.SET_WHITELIST
        , new apk.wire.SetWhiteListRequest(
            apk.wire.WhiteListType.LIST
          , text
          )
        )
        .timeout(10000)
        .then(function(data) {
          var response = apk.wire.SetWhiteListResponse.decode(data)
          if (!response.success) {
            throw new Error('Unable to set whitelist')
          }
        })
    }

    plugin.getWhiteList = function() {
      return runServiceCommand(
          apk.wire.MessageType.GET_WHITELIST
        , new apk.wire.GetWhiteListRequest(
            apk.wire.WhiteListType.LIST
          )
        )
        .timeout(10000)
        .then(function(data) {
          var response = apk.wire.GetWhiteListResponse.decode(data)
          if (response.success) {
            switch (response.type) {
              case apk.wire.WhiteListType.LIST:
                return response.text
            }
          }
          throw new Error('Unable to get clipboard')
